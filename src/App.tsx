...
export default function App() {
  ...
  const [isBookmarksSidebarOpen, setIsBookmarksSidebarOpen] =
    useState(false);
  const [isRequestModalOpen, setIsRequestModalOpen] =
    useState(false);
  const [isDownloadSuccessOpen, setIsDownloadSuccessOpen] =
    useState(false);
  const [isSnackbarOpen, setIsSnackbarOpen] = useState(false);
  const [snackbarMessage, setSnackbarMessage] = useState("");
  const [downloadCount, setDownloadCount] = useState(0);
  const [currentView, setCurrentView] = useState<
    "home" | "dashboard" | "tracking" | "detail"
  >("home");
  ...
  >([
    {
      ...
      description:
        "Membutuhkan data jumlah penduduk per kecamatan untuk penelitian demografi",
      ...
      timeline: [
        {
          ...
          description:
            "Permintaan data telah diterima dan sedang menunggu review",
        },
        {
          status: "under_review",
          timestamp: new Date("2024-11-15T14:30:00"),
          description:
            "Tim kami sedang melakukan review terhadap permintaan Anda",
        },
        {
          status: "processing",
        ...
        },
        {
          ...
          description:
            "Dataset telah berhasil diproses dan siap diunduh",
        },
      ],
    },
    {
      ...
      description:
        "Data sensor kualitas udara untuk analisis pola polusi",
  ...
  const addToCart = (dataset: Dataset) => {
    if (!cartItems.find((item) => item.id === dataset.id)) {
      ...
      setSnackbarMessage(
        `"${dataset.title}" berhasil ditambahkan ke keranjang`,
      );
  ...
  const removeFromCart = (datasetId: number) => {
    setCartItems(
      cartItems.filter((item) => item.id !== datasetId),
    );
  };
  ...
  const handleDownloadAll = (datasets: Dataset[]) => {
    const now = new Date();
    const downloaded: DownloadedDataset[] = datasets.map(
      (dataset) => ({
        ...dataset,
        downloadedAt: now,
  ...
  const handleRequestDatasetSubmit = (
    data: RequestDatasetFormData,
  ) => {
    ...
    addDataRequest({
      ...
      category: "General",
      purpose:
        data.purpose === "Pilih Tujuan"
          ? "Lainnya"
          : data.purpose,
      organization:
        data.occupation === "Pilih Pekerjaan"
          ? "Lainnya"
          : data.occupation,
      email: data.email,
    });
    alert(
      `Permohonan dataset berhasil dikirim!\n\nNama: ${data.name}\nDataset: ${data.datasetTitle}\n\nSilakan cek Lacak Dataset untuk melacak status permohonan Anda.`,
    );
  };
  ...
  const handleSaveDataset = (dataset: Dataset) => {
    if (
      !bookmarkedDatasets.find((item) => item.id === dataset.id)
    ) {
  ...
  const handleAddBookmarkToCart = (dataset: Dataset) => {
    if (!cartItems.find((item) => item.id === dataset.id)) {
      setCartItems([...cartItems, dataset]);
      setSnackbarMessage(
        `"${dataset.title}" ditambahkan ke keranjang`,
      );
      setIsSnackbarOpen(true);
    } else {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <BookmarksSidebar
        ...
        onViewDetail={(dataset) =>
          handleViewDatasetDetail(dataset, "bookmarks")
        }
      />
      <RequestDatasetModal
